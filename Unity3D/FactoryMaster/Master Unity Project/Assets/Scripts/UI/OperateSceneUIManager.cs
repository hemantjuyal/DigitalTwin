// Copyright (c) Microsoft Corporation.
// Licensed under the MIT License.
using Microsoft.MixedReality.Toolkit.UI;
using UnityEngine;

/// <summary>
/// Handles summoning the UI Panels in the Operate Scene
/// </summary>
public class OperateSceneUIManager : MonoBehaviour
{
    [Header("UI Components")]
    [SerializeField]
    private SiteOverviewUIPanel siteOverviewPanel;

    [SerializeField]
    private RoboticPalletizerUIPanel roboticPalletizerPanel;

    [Header("UI Alert Prefabs")]
    [SerializeField]
    private RoboticPalletizerAlert alertPopupPrefab;

    private RoboticPalletizerAlert alertPopup;

    [SerializeField]
    private MessageBoxUI messageBoxPrefab;

    private MessageBoxUI messageBox;

    [SerializeField]
    private GameObject successPopupPrefab;

    private GameObject successPopup;

    private void Start()
    {
        roboticPalletizerPanel.gameObject.SetActive(false);
    }

    /// <summary>
    /// Show the Site Overview panel in the scene
    /// </summary>
    public void ShowSiteOverviewPanel(bool enableFollowMe = true)
    {
        siteOverviewPanel.gameObject.SetActive(true);
        siteOverviewPanel.GetComponent<FollowMeToggle>()?.SetFollowMeBehavior(enableFollowMe);
    }

    /// <summary>
    /// Called when a Robotic Palletizeris selected in the scene or Site Overview Panel
    /// </summary>
    public void OnRoboticPalletizerSelected(RoboticPalletizerScriptableObject roboticPalletizer)
    {
        roboticPalletizerPanel.gameObject.SetActive(true);
        roboticPalletizerPanel.SetRoboticPalletizerData(roboticPalletizer);
    }

    /// <summary>
    /// Shows an alert message for a specific Robotic Palletizer
    /// </summary>
    /// <param name="eventData"></param>
    public void ShowAlertMessageForRoboticPalletizer(ADTRoboticPalletizerEventData eventData)
    {
        if (alertPopup == null)
        {
            alertPopup = Instantiate(alertPopupPrefab);
        }
        Debug.Log("Alert Received at OperateSceneUIManager.cs");
        alertPopup.gameObject.SetActive(true);
        alertPopup.ShowAlertMessageForRoboticPalletizer(eventData);
    }

    public void OnResetCommandSent(RoboticPalletizerScriptableObject roboticPalletizer)
    {
        if (messageBox == null)
        {
            messageBox = Instantiate(messageBoxPrefab);
        }

        messageBox.onDismissed += OnMessageBoxDismissed;
        messageBox.gameObject.SetActive(true);
        messageBox.ShowMessage("Reset Command Sent", $"A reset command has successfully been sent to Robotic Palletizer" +
                                                     $" {roboticPalletizer.roboticPalletizerData.RoboticPalletizerID}");
    }

    private void OnMessageBoxDismissed()
    {
        messageBox.onDismissed -= OnMessageBoxDismissed;
        ShowSuccessMessage();
    }

    /// <summary>
    /// Shows a success message panel
    /// </summary>
    private void ShowSuccessMessage()
    {
        if (successPopup == null)
        {
            successPopup = Instantiate(successPopupPrefab);
        }

        successPopup.gameObject.SetActive(true);
    }
}